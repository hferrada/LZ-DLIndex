...
3.- Edit the Makefile file included in the folder 'LZ-DLIndex' and modify the next lines:  
...
4.- Make: To make the index just execute the command 'make' and this will create the lib: 'DL_LZ.a'
5.- Use the index in your program linking the 'DL_LZ.a' file and include the header "LZDocList64.h" in your sorce code.  
Execute: "make build_binary" to generate a binary file "buildDL_LZ" from the source file "buildDL_LZ.cpp" (included here as example).
buildDL_Sada.cpp creates the index given the parameters included in a config file, which is indicated in the call.  
Execute: "make load_binary" to generate a binary file "loadDL_LZ" from the source file "loadDL_LZ.cpp" (included here as example) 
loadDL_LZ.cpp. This loads the index given the parameters included in the config file, which is indicated in the call.  
